...
static int tdbPageAllocate(SPage *pPage, int szCell, SCell **ppCell) {
  ...
  *ppCell = NULL;
  ...
  if (pPage->pFreeEnd - pPage->pFreeStart != TDB_PAGE_CCELLS(pPage)) {
    tdbError("tdb/page-allocate: ccells: %d, pFreeStart: %p, pFreeEnd: %p.", TDB_PAGE_CCELLS(pPage), pPage->pFreeStart,
             pPage->pFreeEnd);
    return -1;
  }
